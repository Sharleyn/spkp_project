defmodule SpkpProject.KursusTest do
  ...
  describe "kursus_kategori" do
    ...
    test "create_kursus_kategori/1 with valid data creates a kursus_kategori" do
      valid_attrs = %{kategori: "some kategori"}
    ...
    test "update_kursus_kategori/2 with valid data updates the kursus_kategori" do
      kursus_kategori = kursus_kategori_fixture()
      update_attrs = %{kategori: "some updated kategori"}
  ...
  describe "kursus" do
    ...
    test "create_kursuss/1 with valid data creates a kursuss" do
      valid_attrs = %{nama_kursus: "some nama_kursus", tarikh_mula: ~D[2025-08-20], tarikh_akhir: ~D[2025-08-20], tempat: "some tempat", status_kursus: "some status_kursus", had_umur: 42, anjuran: "some anjuran", gambar_anjuran: "some gambar_anjuran", gambar_kursus: "some gambar_kursus", syarat_penyertaan: "some syarat_penyertaan", syarat_pendidikan: "some syarat_pendidikan", kuota: 42, tarikh_tutup: ~D[2025-08-20]}
    ...
    test "update_kursuss/2 with valid data updates the kursuss" do
      kursuss = kursuss_fixture()
      update_attrs = %{nama_kursus: "some updated nama_kursus", tarikh_mula: ~D[2025-08-21], tarikh_akhir: ~D[2025-08-21], tempat: "some updated tempat", status_kursus: "some updated status_kursus", had_umur: 43, anjuran: "some updated anjuran", gambar_anjuran: "some updated gambar_anjuran", gambar_kursus: "some updated gambar_kursus", syarat_penyertaan: "some updated syarat_penyertaan", syarat_pendidikan: "some updated syarat_pendidikan", kuota: 43, tarikh_tutup: ~D[2025-08-21]}

      assert {:ok, %Kursuss{} = kursuss} = Kursus.update_kursuss(kursuss, update_attrs)
      assert kursuss.nama_kursus == "some updated nama_kursus"
      assert kursuss.tarikh_mula == ~D[2025-08-21]
      assert kursuss.tarikh_akhir == ~D[2025-08-21]
      ...
      assert kursuss.tarikh_tutup == ~D[2025-08-21]
    end
